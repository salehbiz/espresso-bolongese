...
interface AwardSectionProps {
  variant?: "white" | "wood-50" | "toasted";
}

export default function AwardSection({ variant = "toasted" }: AwardSectionProps) {
  const awards = [
      ...
      <div className="max-w-[1440px] mx-auto px-5 sm:px-8">
        <div className="flex flex-col lg:flex-row items-center justify-between gap-10 text-center lg:text-left">
          ...
          <div className="max-w-[560px]">
            <h2 className="text-white text-3xl sm:text-4xl md:text-[46px] mb-3">
              An Award-winning escape
            </h2>
            <p className="large-paragraph award-paragraph text-[#FAF7F2]/90">
          ...
          <div className="flex items-center justify-center flex-wrap gap-8 sm:gap-12">
            {awards.map((src, i) => (
              <img
                ...
                className="h-12 sm:h-16 w-auto object-contain transition-opacity duration-300 brightness-0 invert opacity-85 hover:opacity-100"
              />
            ))}
          </div>
        </div>
